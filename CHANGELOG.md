...
- **Service API 调用**：`sdk.services.*` 封装支付、服务商支付、退款、账单、签约、代扣、支付分、收银台、证书等常用接口；通用 `sdk.path()` 继续作为未封装接口兜底。
...
- **示例代码**：`examples/` 包含 Native(RSA/AutoRSA)、App、JSAPI、H5、退款、查单、Flask 回调、FastAPI 回调等完整示例，全部使用环境变量不硬编码敏感信息。
