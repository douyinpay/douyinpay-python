...
def test_sign_request_rsa_produces_headers():
    ...
    assert Headers.Timestamp not in signed.headers
    assert Headers.Nonce not in signed.headers
    assert "DouyinPay-RSA" in signed.headers[Headers.Authorization]
    assert f'timestamp="{signed.timestamp}"' in signed.headers[Headers.Authorization]
    assert f'nonce_str="{signed.nonce}"' in signed.headers[Headers.Authorization]
    assert 'serial_no="ser-1"' in signed.headers[Headers.Authorization]
...
def test_verify_response_bad_signature():
    ...
    with pytest.raises(DouYinPaySignatureError):
        verify_response(headers, "body", {PLAT_SERIAL: CERT_PEM}, SignType.RSA)
