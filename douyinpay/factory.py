    ...
    **kwargs,
    ...
    if not platform_serial:
        try:
            ...
            platform_serial = serial + "_platform"
    certs: Dict[str, KeyLike] = {}
    add_certificate(certs, cert_pem, platform_serial)
    user_sign = kwargs.pop("sign_type", None)
    user_enc = kwargs.pop("encrypt_type", None)
    config = DouYinPayConfig(
        ...
        private_key=private_key,
        certs=certs,
        sign_type=user_sign if user_sign is not None else SignType.SM2,
        encrypt_type=user_enc if user_enc is not None else EncryptType.SM4,
        sdk_agent=kwargs.pop("sdk_agent", SdkAgentType.SM2),
        **kwargs,
    )
    return DouyinPayClient(config)


def create_auto_sm2_client_with_manager(
    mchid: str,
    serial: str,
    private_key: KeyLike,
    encrypt_key: str,
    ...
    **kwargs,
    ...
    manager_kwargs = {
        ...
        "private_key": private_key,
        "encrypt_key": encrypt_key,
        "sign_type": SignType.SM2,
        "encrypt_type": EncryptType.SM4,
        "refresh_interval_sec": refresh_interval_sec,
    }
    for k in ("base_url", "base_uri", "max_clock_offset", "timeout", "http_client", "on_request"):
        if k in kwargs:
    ...
    config = DouYinPayConfig(
        ...
        certificate_provider=manager,
        encrypt_key=encrypt_key,
        sdk_agent=kwargs.pop("sdk_agent", SdkAgentType.AUTO_SM2),
        **kwargs,
    )
    client = DouyinPayClient(config)
    client._certificate_manager = manager
...
def create_auto_sm2_client(
    ...
    **kwargs,
    ...
    return create_auto_sm2_client_with_manager(
        mchid,
        serial,
        private_key,
        encrypt_key,
